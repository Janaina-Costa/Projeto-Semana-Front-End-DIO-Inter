...
export default class PixService{
    ...
    async pay(key:string, user: Partial<User>){
        ...
        if(payingUser?.wallet && payingUser.wallet < Number(keyDecoded.value)){
            throw new AppError('Sem slado suficiente para a transação', 401)
        }
    ...
    async transactions(user: Partial<User>){
        ...
        const pixReceived = await (await pixRepository.find({where:{requestingUser:user.id, status:
        'close'}, relations:['payingUser']}))
